...
def get_bot_service_dicts_for_bots(user_profile):
...
def get_bot_service_dicts_for_users(user_profile):
    # type: (UserProfile) -> List[Dict[str, Any]]

    bots = UserProfile.objects.filter(realm=user_profile.realm, is_bot=True, bot_owner=user_profile)
    service_dicts = []
    for bot in bots:
        service_dicts.extend(get_bot_service_dicts_for_bots(bot))
    return service_dicts
