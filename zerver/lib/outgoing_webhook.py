...
from zerver.models import Realm, Services, get_realm_from_domain
...
class OutgoingWebhookServiceInterface(object):
    ...
    def __init__(self, base_url, service_api_key, bot_email, service_name):
        ...
        self.service_api_key = None # type: Text
...
class ServiceMessageActions(object):
    ...
    def succeed_with_message(cls, event, success_message):
        ...
        realm = get_realm_from_domain(event['message']['sender_domain'])
...
def do_rest_call(bot_interface, rest_operation, trigger_cache, timeout=None):
    ...
    request_kwargs['auth'] = (bot_interface.service_name, bot_interface.service_api_key)
...
def get_outgoing_webhook_service_handler(service):
    ...
    service_interface_class = get_service_interface_class(service.interface)
